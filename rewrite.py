...
# We will completely replace the IntegrationsContent function and keep everything else mostly the same.
# Let's extract everything before function IntegrationsContent()
prefix_match = re.search(r'(.*?)function IntegrationsContent\(\) \{', content, re.DOTALL)
prefix = prefix_match.group(1)

# And everything after the return statement of IntegrationsContent
suffix_match = re.search(r'(export default function IntegrationsPage.*)', content, re.DOTALL)
suffix = suffix_match.group(1)

# Now we construct the new IntegrationsContent
new_body = '''function IntegrationsContent() {
