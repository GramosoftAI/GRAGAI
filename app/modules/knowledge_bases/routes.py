            ...
            try:
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    await db.execute(
                        ...
                        .values(file_hash=file_hash, parsed_path=dataset_name, description="excel_parquet", categorical_values=categorical_registry, dataset_schema=schema_registry)
                    )
                    await db.commit()
...
async def register_google_drive(
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if kb_obj:
                if not kb_obj.name or "(google drive)" not in kb_obj.name.lower():
                    kb_obj.name = "Google Drive Knowledge"
                    db.add(kb_obj)
            ...
            try:
                from .service import KnowledgeBaseService
                service = KnowledgeBaseService(db, tenant_id)
                await service.neo4j_repo.execute_write(
                    """
                    MATCH (kb:KnowledgeBase {id: $kb_id, tenant_id: $tenant_id})
                    SET kb.name = 'Google Drive Knowledge'
                    """,
                    {"kb_id": kb_id, "tenant_id": str(tenant_id)}
                )
            except Exception as neo_err:
                logger.warning(f"Failed to update Neo4j KB name during register: {neo_err}")
...
async def register_sharepoint(
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            kb_obj = kb_res.scalar_one_or_none()
            if kb_obj:
                if not kb_obj.name or "(sharepoint)" not in kb_obj.name.lower():
                    kb_obj.name = "SharePoint Knowledge"
                    db.add(kb_obj)
            ...
            try:
                ...
                await service.neo4j_repo.execute_write(
                    """
                    MATCH (kb:KnowledgeBase {id: $kb_id, tenant_id: $tenant_id})
                    SET kb.name = 'SharePoint Knowledge'
                    """,
                    {"kb_id": kb_id, "tenant_id": str(tenant_id)}
                )
            except Exception as neo_err:
                logger.warning(f"Failed to update Neo4j KB name during sharepoint register: {neo_err}")
            return format_success(
                {"success": True},
